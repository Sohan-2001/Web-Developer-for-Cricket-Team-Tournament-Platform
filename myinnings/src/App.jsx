...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans">
      ...
      <header className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur-md border-b border-slate-800">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-lg font-black text-white tracking-tight leading-tight">MyInnings</h1>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-medium -mt-0.5">Cricket Match OS</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-slate-800 border border-slate-700 rounded-full flex items-center justify-center text-xs font-bold text-emerald-400">
              SK
            </div>
          </div>
        </div>
      ...
      <nav className="sticky top-[60px] z-40 bg-slate-900/90 backdrop-blur-md border-b border-slate-800">
          ...
          <div className="flex">
            {tabs.map((tab, index) => {
              ...
              return (
                ...
                >
                  {/* Step number badge */}
                  <span className={`hidden sm:flex w-5 h-5 rounded-full items-center justify-center text-[10px] font-bold shrink-0 ${
                    isActive
                  ...
                  <Icon className={`w-4 h-4 sm:hidden shrink-0 ${isActive ? 'text-emerald-400' : 'text-slate-600'}`} />
                  ...
                  {/* Connector chevron */}
                  {index < tabs.length - 1 && (
                    <ChevronRight className="absolute -right-1.5 w-3 h-3 text-slate-700 hidden sm:block" />
                  )}
                </button>
              );
            })}
      ...
      <div className="max-w-5xl mx-auto px-4 pt-4">
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs sm:text-sm text-slate-400">
              Active: <span className="text-white font-medium">Pune Strikers CC vs RC Pune</span>
            </span>
          </div>
          <span className="text-[10px] sm:text-xs text-slate-600">Sun, 7 Sep</span>
        </div>
      </div>
